...
- [ ] Link local repo to GitHub repo you already created.
- [ ] Add remote: `git remote add origin <github-repo-url>` (or update existing remote URL).
- [ ] Verify remote: `git remote -v`.
- [ ] Push bootstrap branch: `git push -u origin main` (or chosen default branch).
- [ ] Create baseline branches: `main`, `staging`, `dev`.
...
- [ ] Add `.editorconfig`, formatter config, and shared lint config.
- [ ] Add CI workflow skeleton (lint, test, build placeholders).

### 14.2 Tauri + React Monorepo Foundation
- [ ] Initialize Tauri v2 app with React + TypeScript.
- [ ] Define folder structure for:
- [ ] Folder target: `src-ui/` (React app).
- [ ] Folder target: `src-tauri/` (Rust commands/services).
- [ ] Folder target: `packages/core/` (schemas, IR, validators).
- [ ] Folder target: `packages/exporter-astro/`.
- [ ] Folder target: `packages/lint-rules/`.
- [ ] Configure pnpm/npm workspace.
- [ ] Set TypeScript project references.
- [ ] Add Rust formatter/lints (`rustfmt`, `clippy`) config.
- [ ] Add pre-commit hooks (format + lint).
- [ ] Confirm app boots on macOS and opens blank shell UI.

### 14.3 Project Schema and Persistence
- [ ] Implement V1 schema files: `project.json`, `site.json`, `theme.json`, `pages/*`, `content/*`, `blocks.lock.json`.
- [ ] Implement semver schema version in `project.json`.
- [ ] Build deterministic JSON serializer (stable key ordering).
- [ ] Implement create/open/save project flows.
- [ ] Implement autosave with debounce and conflict-safe writes.
- [ ] Implement `.cmsproj` packaging/unpackaging.
- [ ] Implement migration runner (forward-only).
- [ ] Implement backup creation at `backups/<timestamp>-pre-migration/`.
- [ ] Log migrations to `exports/migrations.log`.
- [ ] Add schema fixtures for migration tests.

### 14.4 Canonical IR and Runtime Engine
- [ ] Define TypeScript types for `PageIR`, `BlockInstanceIR`, `ThemeIR`.
- [ ] Implement normalization pipeline (defaults, strip unknowns, validation).
- [ ] Implement deterministic `instanceId` generation using locked strategy.
- [ ] Implement token resolution into `ThemeIR`.
- [ ] Implement runtime block renderer interface bound to IR only.
- [ ] Add snapshot tests for normalized IR output.
- [ ] Add regression tests proving reorder does not regenerate IDs.

### 14.5 Block System (Internal + Workspace Library)
- [ ] Create internal block registry and loader.
- [ ] Implement workspace watcher for `project/blocks/**`.
- [ ] Implement manifest parser for `block.manifest.json`.
- [ ] Validate manifest required fields (`blockId`, schemas, contracts).
- [ ] Add block catalog merge logic (internal + workspace).
- [ ] Add block load error panel with actionable diagnostics.
- [ ] Implement delete behavior for missing workspace blocks (flag existing instances).
- [ ] Add dependency allowlist parser from `project.json`.
- [ ] Enforce exact version pin policy and conflict detection.
- [ ] Add tests for add/update/delete hot-load events.
...
### 14.9 Astro Exporter
- [ ] Scaffold `packages/exporter-astro/`.
...
- [ ] Unit tests for schema, migration, IR normalization, ID generation.
- [ ] Integration tests for project save/load/package/import.
- [ ] Integration tests for workspace block hot-load.
